...
function LogueItem({ id, price, no, title, cart, updateCart, setLocalCart, localCart, setDirty }) {
  ...
  return (
    <div className="flex justify-between items-center bg-white sm:bg-white border-gray-300 border-b border-collapse">
      ...
      <span className="w-20p text-center flex items-center justify-center">${price.toFixed(2)}</span>
      <div className="w-20p flex justify-center">
        <input className="w-10 h-6 text-center flex justify-center items-center border-gray-200 border" type="number" value={no} onChange={(event) => handleChange(+event.target.value, id)} />
      </div>
