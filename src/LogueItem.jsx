...
function LogueItem({ id, price, no, title, cart, updateCart, setLocalCart, localCart, setDirty }) {
  ...
  const handleChange = (newVal, productId) => {
    setDirty(true);
  ...
  return (
    <div className="flex justify-between items-center bg-white border-gray-300 border-b">
      <span className="flex items-center w-40p sm:w-50p text-start">
        <button className="p-4" onClick={() => handleRemove(id)}>
        ...
        <img className="w-20 h-16 py-[4px]" src="https://images.pexels.com/photos/2536965/pexels-photo-2536965.jpeg?auto=compress&cs=tinysrgb&w=600" alt={title} />
        <p className="ml-4 text-red-600 font-bold hidden sm:block w-full truncate">{title}</p>
      </span>
      ...
      <span className="w-[15%] text-start flex items-center justify-center hidden sm:block">${(price * no).toFixed(2)}</span>
    </div>
  );
}
