...
function LogueItem({ id,image,price,no,title,cart,updateCart,setLocalCart,localCart,setDirty }){
  ...
  return(
    <div className='flex justify-between items-center bg-white sm:bg-white border-gray-300 border-b border-collapse'>
      <span className='flex items-center w-40p sm:w-50p text-center'>
        <button className="p-4" onClick={function (event){
            handleRemove(id);
            }}>
        ...
        <img className="w-20 h-16 py-[4px]" src={image} alt={title} />
        <p className="hidden sm:block pl-20 text-red-600 font-bold">{title}</p>
      </span>
